...
uintptr_t GetPlayerHeadTF(uintptr_t player) {
    auto ptr = Read<uintptr_t>(player + 0x658); // Player.CLOEKEADCHD (ITransformNode head) [dump.cs]
    return ptr;
}

uintptr_t GetPlayerPeTF(uintptr_t player) {
    auto ptr = Read<uintptr_t>(player + 0x688); // Player.INHGPBHOKPF (ITransformNode foot) [dump.cs]
    return ptr;
}
